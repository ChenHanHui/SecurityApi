...
public class RSATest {
    ...
    public static void encryptAndDecrypt() {
        ...
        System.out.println("获取到的私钥：" + privateKey);
        // 待加密数据
    ...
    public static void signAndVerify() {
        ...
        String sign = RSAUtils.sign(data.getBytes(), Base64.getDecoder().decode(privateKey), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("数字签名结果：" + sign);
        boolean verify = RSAUtils.verify(data.getBytes(), Base64.getDecoder().decode(sign), Base64.getDecoder().decode(publicKey), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("数字签名验证结果：" + verify);
    }

    public static void verifyAndDecrypt() {
        ...
        String encrypt = RSAUtils.encryptByPublicKey(data, serverPublicKeyStr);
        System.out.println("客户端使用服务器公钥加密：" + encrypt);
        String sign = RSAUtils.sign(encrypt.getBytes(), Base64.getDecoder().decode(clientPrivateKeyStr), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("客户端将加密数据用客户端私钥签名：" + sign);
        boolean verify = RSAUtils.verify(encrypt.getBytes(), Base64.getDecoder().decode(sign), Base64.getDecoder().decode(clientPublicKeyStr), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("服务器使用客户端公钥数字签名验证结果：" + verify);
        if (verify) {
            System.out.println("服务器使用客户端公钥数字签名验证通过，开始解密数据");
            String decrypt = RSAUtils.decryptByPrivateKey(encrypt, serverPrivateKeyStr);
    ...
    public static void isValidSignature() {
        ...
        String data = "一碗情深";
        System.out.println("客户端原始数据：" + data);
        String encrypt = RSAUtils.encryptByPrivateKey(data, clientPrivateKeyStr);
        ...
        // 客户端使用私钥对摘要进行签名
        String sign = RSAUtils.sign(hash.getBytes(), Base64.getDecoder().decode(clientPrivateKeyStr), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("客户端使用客户端私钥对数据摘要进行签名，生成数字签名：" + sign);
        // 服务器使用客户端公钥对摘要进行验证签名，确保数据的完整性和来源的真实性
        boolean verify = RSAUtils.verify(hash.getBytes(), Base64.getDecoder().decode(sign), Base64.getDecoder().decode(clientPublicKeyStr), RSAUtils.RSA256_SIGNATURE_ALGORITHM);
        System.out.println("服务器使用客户端公钥验证签名结果：" + verify);
        if (verify) {
            System.out.println("数据摘要签名验证成功，数据有效");
            String decrypt = RSAUtils.decryptByPublicKey(encrypt, clientPublicKeyStr);
            System.out.println("服务器使用客户端公钥对加密数据解密：" + decrypt);
        }
    }

    public static void clientEncryptSignature() {
