...
public class Convert {
    ...
    static {
        ...
        CONVERTERS.put(Short.TYPE, Short::valueOf);
        CONVERTERS.put(Byte.class, Byte::valueOf);
        CONVERTERS.put(Byte.TYPE, Byte::valueOf);
        ...
        CONVERTERS.put(Character.TYPE, s -> s.charAt(0));
    }

    @SuppressWarnings("unchecked")
    public static <T> T getValueByType(Object obj, Class<T> targetClass) {
        if (obj == null) {
            return null;
        }
        ...
        try {
            ...
            if (converter != null) {
                return (T) converter.apply(objStr);
            } else {
                // 对于未映射的类型，抛出异常或返回默认值，这里选择抛出异常
                throw new IllegalArgumentException("Unsupported target class: " + targetClass.getName());
            }
        } catch (Exception e) {
            // 对于转换失败的情况，可以选择记录日志、抛出运行时异常或返回默认值，这里选择抛出运行时异常
            throw new RuntimeException("Failed to convert object to " + targetClass.getName(), e);
        }
    }
    ...
    public static Character toChar(Object value, Character defaultValue) {
        ...
        return StringUtils.isNull(valueStr) ? defaultValue : valueStr.charAt(0);
    }
    ...
    public static String str(byte[] bytes, String charset) {
        return str(bytes, StringUtils.isNull(charset) ? Charset.defaultCharset() : Charset.forName(charset));
    }
